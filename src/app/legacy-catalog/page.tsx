...
/**
 * The legacy PDF catalogue.
 *
 * `/catalogue` is now the interactive experience-catalogue (a static bundle in
 * public/catalogue). This PDF viewer moved here to `/legacy-catalog` so the
 * downloadable PDF is still hosted for anyone who has the link, but it is
 * deliberately NOT linked from anywhere on the site and NOT in the sitemap, and
 * carries a noindex robots directive. (On the customer domain the platform
 * strips head metas, so the real protection is unlinked + not-in-sitemap; the
 * meta is kept anyway for canary/workers.dev and correctness.)
 */
export const metadata: Metadata = {
  title: 'Catalogue (PDF) — Woodlark Hybrid Veneer',
  description:
    'The downloadable Woodlark hybrid veneer catalogue (PDF) — the full palette of tones and finishes. All designs available in 8x4 and 10x4.',
  robots: { index: false, follow: false },
  alternates: { canonical: siteUrl('/legacy-catalog') },
  openGraph: { url: siteUrl('/legacy-catalog'), images: [siteConfig.ogImage] },
};

const { catalogue } = siteConfig;
const hasCatalogue = Boolean(catalogue.url);

export default function LegacyCataloguePage() {
  return (
    <>
      {/* Intro */}
